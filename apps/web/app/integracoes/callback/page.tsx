...
function Callback() {
  ...
  useEffect(() => {
    ...
    if (recusado || !code || !provedor) {
      ...
      router.replace(`/painel/contatos?erro=${encodeURIComponent(motivo)}`);
      return;
    }

    void (async () => {
      const { data, error } = await api.POST("/api/v1/integracoes/contatos/callback", {
        body: {
          provedor,
          code,
          state,
      ...
      if (error || !data) {
        ...
        setErro(detalhe);
        setTimeout(
          () => router.replace(`/painel/contatos?erro=${encodeURIComponent(detalhe)}`),
          2500,
        );
        return;
      }
      const conta = (data as { conta: string }).conta;
      router.replace(`/painel/contatos?conectado=${encodeURIComponent(conta)}`);
    })();
  }, [params, router]);
