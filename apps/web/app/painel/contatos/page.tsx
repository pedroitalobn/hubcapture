...
export default function ContatosPage() {
  ...
  const carregarContatos = useCallback(async (termo: string) => {
    const { data } = await api.GET("/api/v1/contatos", {
  ...
  const carregarIntegracoes = useCallback(async () => {
    const [{ data: provs }, { data: ints }] = await Promise.all([
      api.GET("/api/v1/integracoes/contatos/provedores"),
      api.GET("/api/v1/integracoes/contatos"),
    ]);
  ...
  // volta do consentimento OAuth (/integracoes/callback redireciona para cá)
  useEffect(() => {
  ...
  async function sincronizar(integracaoId?: string) {
    ...
    const resposta = integracaoId
      ? await api.POST("/api/v1/integracoes/contatos/{integracao_id}/sync", {
          params: { path: { integracao_id: integracaoId } },
        })
      : await api.POST("/api/v1/integracoes/contatos/sync", {});
  ...
  async function alterarDirecao(integracao: Integracao, direcao: string) {
    await api.PATCH("/api/v1/integracoes/contatos/{integracao_id}", {
  ...
  async function desconectar(integracao: Integracao) {
    if (
      ...
      return;
    await api.DELETE("/api/v1/integracoes/contatos/{integracao_id}", {
      params: { path: { integracao_id: integracao.id } },
    });
    await carregarIntegracoes();
  }
  ...
  async function arquivar(contato: Contato) {
    ...
    await api.DELETE("/api/v1/contatos/{contato_id}", {
...
function FormularioContato({
  ...
  async function enviar(e: React.FormEvent) {
    ...
    if (contato) {
      await api.PATCH("/api/v1/contatos/{contato_id}", {
        params: { path: { contato_id: contato.id } },
        body: corpo,
      });
    } else {
      await api.POST("/api/v1/contatos", { body: corpo });
    }
    setSalvando(false);
    await onSalvo();
  }
